...
select @names = isnull(@names + ',', '') + QUOTENAME(CustomerName) 
	from (
	select CustomerName as CustomerName
	from Sales.Customers as cus) as CustomerNames;
...
select convert(varchar, beginMonth, 104) as ''InvoiceMonth'', ' + @names + '
from  datatable
PIVOT (sum(Number)
